      ...
      if (statementHandler != null) {
         ...
         if (mappedStatement.getId().matches(pageSqlId)) {
            ...
            if (arr.length > 1) {
               ...
               count = sqlLower.indexOf(" from ");
               countSql = "select count(1) " + sqlLower.substring(count);
               ...
               myCatZhujie = sql.toLowerCase();
               int fromIndex = myCatZhujie.indexOf(" from ");
               countSql = "select count(1) " + myCatZhujie.substring(fromIndex);
               int orderIndex = countSql.lastIndexOf(" order ");
               if (orderIndex >= 0) {
                  count = countSql.lastIndexOf(")");
                  if (count < orderIndex) {
                     countSql = countSql.substring(0, orderIndex);
                  }
               }
      ...
      if (page != null && notEmpty(dialect)) {
         ...
         if (start != null && limit != null) {
            if ("mysql".equals(dialect)) {
               ...
               pageSql.append((long)page.getStart() + (long)page.getTotalPage());
